        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if "multi_modal_inputs" in batch.non_tensor_batch.keys():
                    gen_batch = batch.pop(
                        batch_keys=['input_ids', 'attention_mask', 'position_ids'],
                        non_tensor_batch_keys=['raw_prompt_ids', 'multi_modal_data', 'origin_multi_modal_data', 'multi_modal_inputs', 'extra_info'],
                    )
                else:
                    gen_batch = batch.pop(
                        batch_keys=["input_ids", "attention_mask", "position_ids"],
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.config.get("vs_debug", False) or self.config.get("debug", False):
                        try:
                            prompt_ids = gen_batch_output.batch["prompts"].cpu().tolist()
                            resp_ids = gen_batch_output.batch["responses"].cpu().tolist()
                            ...
                            for idx, (p_text, r_text) in enumerate(zip(prompt_texts, resp_texts)):
                                print(
                                    f"\n==================== Rollout {self.global_steps}.{idx:03d} ===================="
                                )
                                print("[Prompt]")
                                print(p_text)
                                print("\n[Response]")
                            ...
                            print(f"[vs_debug] logging failed: {e}")
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        with _timer("reward_calculation", timing_raw):
                            try:
                ...
                # Compute GRPO in-group statistics if using GRPO
                if self.config.algorithm.adv_estimator == AdvantageEstimator.GRPO:
                    metrics.update(compute_grpo_ingroup_metrics(
                        batch=batch,
                        num_repeat=self.config.actor_rollout_ref.rollout.n
                    ))
