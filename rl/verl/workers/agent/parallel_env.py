...
def agent_rollout_loop(config, vllm_engine, vllm_inputs, prompts, multi_modal_inputs, sampling_params):
    ...
    mm_input_list = []
    tool_call_cnt_list = []
    generated_images_list = []  # Track generated PIL Images per rollout (already loaded)
    ...
    for i in range(batch_size):
        for _ in range(sampling_params.n):
            ...
            generated_images_list.append([])  # Initialize empty list for each rollout
    ...
    for step in range(config.agent.max_turns):
        ...
        observations, rewards, dones, info_list = obs_results


        for idx, obs, act, rew, done, info in zip(active_indices, observations, actions, rewards, dones, info_list):
            # Collect generated PIL Images directly from obs (more reliable than reloading from paths)
            if isinstance(obs, dict) and 'multi_modal_data' in obs and 'image' in obs['multi_modal_data']:
                # PIL Images are already loaded - collect them for reward calculation
                generated_images_list[idx].extend(obs['multi_modal_data']['image'])
    ...
    # Prepare non_tensors dict
    non_tensors = {}
    if processor is not None:
        non_tensors["multi_modal_inputs"] = mm_input_list
    # Store generated PIL Images directly (already loaded during rollout)
    # Convert to numpy object array, ensuring 1D shape for proper concatenation
    generated_images_array = np.empty(len(generated_images_list), dtype=object)
    for i, images in enumerate(generated_images_list):
        generated_images_array[i] = images
    non_tensors["generated_images"] = generated_images_array
...
class ParallelEnv:
    ...
    def step(self, active_indices, actions):
        ...
        done_list = []
        info_list = [{}] * len(actions)  # Track info dict from each tool execution
        valid_indices = []
        real_indices = []
        valid_actions = []
    ...
    def reset(self, prompts, vllm_inputs, n=1, **kwargs):
        ...
        for i in range(len(prompts)):
            ...
            # Use .get() instead of .pop() to avoid corrupting batch-level numpy arrays
            # The batch-level cleanup happens later in vllm_rollout_spmd.py
            ...
            for _ in range(n):
                if tool_name:
                    ...
                    tool_kwargs = {}
                    if hasattr(self.config, 'aux_img_dir') and self.config.aux_img_dir:
                        tool_kwargs['aux_img_dir'] = self.config.aux_img_dir
